...
return new Promise((resolve,reject)=>{
  ...
  if(config){
        ...
        let zuora_signature_full_url = zuora_base_url + zuora_rsa_signature;
...
router.post('/', function(req, res, next) {
  const prepopulate = fs.readFileSync("./conf/prepopulate.json","utf-8");
  let config = fs.readFileSync("./conf/config.json","utf-8");
...
function preparePrePopulatefields(prepopulatedata, configjson) {
  ...
  for (let i = 0; i < mainkeys.length; i++){
    ...
    for (let j = 0; j < subkeys.length; j++) {
      ...
      var encrypted = crypto.publicEncrypt({key:publickKeyObject, padding : crypto.constants.RSA_PKCS1_PADDING}, Buffer.from(buffer)).toString("base64");
